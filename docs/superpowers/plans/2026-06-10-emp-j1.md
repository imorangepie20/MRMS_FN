...
## As-built 차이 (2026-06-10, J1 완료 + 핫픽스 반영)

> J1 구현 완료 후 핫픽스가 연달아 들어가면서 아래 Task들의 코드 블록 다수가 실제 구현과 다름.
> **이 표가 기준** — 본문 Task의 코드/테스트 블록은 작성 시점 스냅샷으로만 참고할 것.
> as-built 상세는 spec(as-built 갱신됨)과 실제 소스 참조.

| 플랜 위치 | 플랜 내용 (구버전) | 실제 구현 (as-built) |
|---|---|---|
| Task 1 | `source_id TEXT` (nullable), `idx_empsource_track` 인덱스, AFTER INSERT 전용 트리거 | `source_id TEXT NOT NULL`, 인덱스 없음, `AFTER INSERT OR DELETE` 트리거 — DELETE 시 마지막 EMPSource row가 사라지면 `Track.inEmp = FALSE` 리셋. → `prisma/migrations/20260610100000_add_emp/migration.sql` |
| (플랜에 없음) | — | 추가 마이그레이션 2개: `20260610200000_add_setting` (Setting), `20260610300000_add_emp_section` (EMPSection/EMPSectionItem) — Tidal 토큰/소스 설정 + /emp 브라우즈용 (hotfix) |
| Task 2/3 | `upsert_emp_source` / `upsert_track_and_emp_source`의 `source_id: str \| None` (+ `source_id or ""` 해시) | `source_id: str` 필수 (컬럼 NOT NULL). 또한 `_get_or_create_artist`는 `nameNormalized` 컬럼도 INSERT, `_id`는 `usedforsecurity=False` 사용 |
| Task 4 | `TidalEMPImporter(client_id, client_secret)` + auth.tidal.com client_credentials (`_get_access_token`) + openapi.tidal.com/v2 + `DEFAULT_PLAYLISTS`(tidal_rising/tidal_discovery) | **전면 교체**: `TidalEMPImporter(conn, token=None)` — 토큰은 Setting `'tidal_x_token'` (없으면 graceful skip), tidal.com 웹 API (`/v2/home/pages/{S}/view-all`, `/v1/playlists/{uuid}/items`, `/v1/pages/album`, `/v1/pages/mix`), 소스는 Setting `'tidal_emp_sources'` (`home/playlist/album/mix` 라인, 기본 `DEFAULT_SOURCES` = home 섹션 4개), `import_all` override가 EMPSection/EMPSectionItem도 기록, 분류/추출은 `_classify_item`/`_walk_classify`/`_walk_tracks`, 커버는 MEDIUM(640px) 우선, source_type `editorial_album`/`editorial_mix` + source_id `'{kind}:{ident}'` 추가. → `src/mrms/emp/tidal.py`. 테스트도 교체: `tests/emp/test_tidal.py`는 classify/cover/`_load_sources`/no-token 단위 테스트 |
| Task 5 | `fetch_editorial_playlists`가 `/browse/featured-playlists` 호출 (+ `test_fetch_featured_playlists`) | Spotify가 2024-11 신규 앱에 featured-playlists **차단** → 하드코딩 `DEFAULT_PLAYLISTS` 10개 + env `SPOTIFY_EMP_PLAYLISTS` override, browse 호출 없음. client_credentials는 트랙 fetch에만 사용. 테스트: `test_default_playlists_returned` / `test_env_override` |
| Task 6/8 | `TidalEMPImporter(client_id=os.environ['TIDAL_CLIENT_ID'], client_secret=...)` | `TidalEMPImporter(conn=conn)` — 토큰 Setting 자동 로딩. `TIDAL_CLIENT_ID`/`TIDAL_CLIENT_SECRET` env는 EMP 경로에서 더 이상 사용 안 함 (배포 env 문서에서도 EMP 요건 아님) |
| Task 7 | `--limit` default=1000, 모듈 상단 `parse_known_args` 스케치 | `--limit` default=0 ("테스트 모드: 첫 N개만", `--emp-only` 시 0이면 DB LIMIT 1000), 기존 argparse/main 흐름에 통합. runner는 `--limit 500` 명시 전달 |
| Task 11 | stats/runs/trigger만 | + `GET/PUT /api/admin/emp/settings` (hotfix) — whitelist `ALLOWED_SETTING_KEYS` = tidal_x_token/tidal_emp_sources, 토큰은 `'…'+마지막 4자` 마스킹, value null → 삭제 |
| Task 12/13 | EmpStats/IngestionRun 타입 + EmpDashboard(stats/runs/trigger) | + `EmpSettings`/`EmpSection`/`EmpSectionItem`/`EmpItemTrack` 타입(`web/src/lib/types.ts`), EmpDashboard에 `SettingsCard`(토큰/소스 편집) + `SectionsTree`, `web/src/lib/api/emp.ts` 헬퍼 (hotfix) |
| Task 14 | Settings 그룹 "EMP admin"(S3)만 | + Sections 그룹에 `{ title: "EMP", href: "/emp", num: "§ 02", badge: "2.4k" }` (사용자용 /emp 브라우즈 진입점) |
| (플랜에 없음) | — | 사용자용 **/emp 브라우즈 기능** 일체 (hotfix): `src/mrms/api/emp_browse.py` (GET /api/emp/sections, GET /api/emp/items/{type}/{id}/tracks), `web/src/app/(dashboard)/emp/page.tsx`, `web/src/components/emp/` (EmpBrowse/SectionRow/ItemTracksModal 등) |
| 파일 구조 | 좌측 트리 | 추가됨: `src/mrms/db/settings.py`, `src/mrms/db/emp_section.py`, `src/mrms/api/emp_browse.py`, `web/src/lib/api/emp.ts`, `web/src/app/(dashboard)/emp/page.tsx`, `web/src/components/emp/*`, `tests/db/{test_emp,test_settings,test_emp_section}.py`, `tests/api/test_emp_browse.py`, 마이그레이션 `20260610200000_add_setting`/`20260610300000_add_emp_section` |
| Self-Review | "Tidal fallback DEFAULT_PLAYLISTS로 대응" 등 | 작성 시점 기준이라 **무효** — 실제 해결은 tidal.com 웹 API 전환 + Setting 기반 소스/토큰이었고, 브라우즈 기능·settings 엔드포인트가 계획 외로 추가됨. 본 표가 우선 |
