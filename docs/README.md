...
- [decisions/ADR-011-flo-section-dedup.md](decisions/ADR-011-flo-section-dedup.md): FLO 섹션 중복 제거 + v2 홈 패널 추가 — sectionKey를 title 기반으로 dedup, 회전식 v1 prune는 철회(파괴적), FLO 웹앱이 쓰는 v2 `/recommends/home/panels`(`POPULAR_CHANNEL` 인라인 트랙)를 추가 소스로 도입해 섹션 확보
